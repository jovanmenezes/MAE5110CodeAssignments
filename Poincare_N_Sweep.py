...
for idx, N in enumerate(N_values):
    ...
    ax.plot(fixed_point, fixed_point, marker='*', ms=6, color='magenta', label=f"$\\dot{{\\theta}}^*$: {fixed_point:.4f} (Slope: {slope:.2f})")
    ax.set_title(f"N = {N}", fontsize=11)
    ax.set_xlabel(r"$\dot{\theta}_k^+$", fontsize=9)
    ax.set_ylabel(r"$\dot{\theta}_{k+1}^+$", fontsize=9)
    ax.grid(True)
    ax.set_aspect('equal', adjustable='box')
    ax.legend(fontsize=7, loc="upper left")

plt.suptitle("Poincaré Return Maps for Rimless Wheel Across Different Spokes ($N$)", fontsize=14)
plt.tight_layout()
plt.savefig("Poincare_N_Sweep.png", dpi=300)
plt.show()
plt.close()