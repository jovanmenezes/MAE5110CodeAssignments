...
def classify_state(args):
    initial_state, gamma, N = args
    params = generate_params(gamma, N)
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        for idx, gamma in enumerate(gamma_values):
            ...
            grid_points = [(np.array([t, o]), gamma, N) for t, o in zip(theta_pts, omega_pts)]
            ...
            ax.scatter(theta_pts[rest_mask], omega_pts[rest_mask], color='blue', s=8, label='Rest State', alpha=0.7)
            ax.scatter(theta_pts[limit_cycle_mask], omega_pts[limit_cycle_mask], color='red', s=8, label='Walking Cycle', alpha=0.7)
            ...
            ax.set_xlabel(r"Initial Angle $\theta_0$ (rad)", fontsize=5)
            ax.set_ylabel(r"Initial Angular Velocity $\dot{\theta}_0$ (rad/s)", fontsize=5)
            ax.legend(loc="upper left", fontsize=6)
            ax.grid(True, alpha=0.3)

    plt.suptitle("Region of Attraction Plots for Rimless Wheel Across Different Slopes ($\gamma$)", fontsize=8)
    plt.tight_layout()
    plt.savefig("region_of_attraction_gamma_sweep.png", dpi=300)
    plt.show()
    plt.close()
